...
//          1234567890123456789012345678901234567890123456789012345678901234
   ...
   if (args[NEWCHARS].uiCount)
      for (i=0; args[NEWCHARS].pszParam[i]; i++)
         pCharMap[i] = args[NEWCHARS].pszParam[i];
   }
...
char CharFromIdx (USHORT pixel)
   {
   if (args[BINARY].uiCount)
      return Bin [pbmp->pGrayMap [pixel] >= 128];
      }
...
char OutChar (PBMPINFO pbmp, PSZ pszLine, USHORT x, USHORT y)
   ...
   switch (pbmp->uBPP)
      {
      case 1:  return Bin [pszLine[x/8]  & (1 << (7 - x % 8))];
      case 4:  return CharFromIdx (pszLine[x/2] & 0x0F << (x % 2));
      case 8:  return CharFromIdx (pszLine[x]);
      case 24:
...
void ProcessBmp (PSZ pszInFile, PSZ pszOutFile)
   ...
   if (!args[INFO].uiCount && !args[SHOWCOLORS].uiCount)
      {
...
void ParseArgs (int argc, char *argv[], PSZ pszInFile, PSZ pszOutFile)
   ...
   if (!args[INFO].uiCount && !args[SHOWCOLORS].uiCount)
      {
      if (uOutFile != 1000 || argv[uOutFile] == NULL)
         {
